/*
 * $Id: ContentTypeHandlerManagerTest.java 676195 2008-07-12 15:55:58Z mrdon $
 *
...
import com.mocha.bsm.bizsm.struts2rest.DefaultContentTypeHandlerManager;
import com.mocha.bsm.bizsm.struts2rest.DefaultHttpHeaders;
import com.mocha.bsm.bizsm.struts2rest.IContentTypeHandlerManager;
import com.mocha.bsm.bizsm.struts2rest.handler.FormUrlEncodedHandler;
...
public class ContentTypeHandlerManagerTest extends TestCase {
    ...
    public void testHandleResultOK() throws IOException {
        ...
        String obj = "mystring";
        IContentHandler handler = new IContentHandler() {
    ...
    public void testHandleResultNotModified() throws IOException {
        ...
        Mock mockHandlerXml = new Mock(IContentHandler.class);
        mockHandlerXml.matchAndReturn("getExtension", "xml");
        mgr.handlersByExtension.put("xml", (IContentHandler) mockHandlerXml.proxy());
    ...
    public void testHandlerOverride() {
        ...
        Mock mockHandlerJson = new Mock(IContentHandler.class);
        ...
        Mock mockHandlerXmlOverride = new Mock(IContentHandler.class);
        ...
        mockContainer.matchAndReturn("getInstance", C.args(C.eq(IContentHandler.class), C.eq("xmlOverride")), mockHandlerXmlOverride.proxy());
        mockContainer.matchAndReturn("getInstance", C.args(C.eq(IContentHandler.class), C.eq("xml")), mockHandlerXml.proxy());
        mockContainer.matchAndReturn("getInstance", C.args(C.eq(IContentHandler.class), C.eq("json")), mockHandlerJson.proxy());
        mockContainer.expectAndReturn("getInstanceNames", C.args(C.eq(IContentHandler.class)), new HashSet(Arrays.asList("xml", "xmlOverride", "json")));

        mockContainer.matchAndReturn("getInstance", C.args(C.eq(String.class),
                C.eq(IContentTypeHandlerManager.STRUTS_REST_HANDLER_OVERRIDE_PREFIX+"xml")), "xmlOverride");
        mockContainer.expectAndReturn("getInstance", C.args(C.eq(String.class),
                C.eq(IContentTypeHandlerManager.STRUTS_REST_HANDLER_OVERRIDE_PREFIX+"json")), null);
        
        DefaultContentTypeHandlerManager mgr = new DefaultContentTypeHandlerManager();
        mgr.setContainer((Container) mockContainer.proxy());

        Map<String,IContentHandler> handlers = mgr.handlersByExtension;
    ...
    public void HandleRequestContentType() throws IOException {
        ...
        Mock mockHandlerForm = new Mock(IContentHandler.class);
        ...
        Mock mockContainer = new Mock(Container.class);
        mockContainer.matchAndReturn("getInstance", C.args(C.eq(IContentHandler.class), C.eq("x-www-form-urlencoded")), mockHandlerForm.proxy());
        mockContainer.expectAndReturn("getInstanceNames", C.args(C.eq(IContentHandler.class)), new HashSet(Arrays.asList("x-www-form-urlencoded", "json")));

        mockRequest.setContentType(FormUrlEncodedHandler.CONTENT_TYPE);
        mockRequest.setContent("a=1&b=2".getBytes("UTF-8"));
        IContentHandler handler = mgr.getHandlerForRequest(mockRequest);
