...
function index_gridpanel_DomStruFn(conf){
		  ...
		  for(var i=0,len=trs.length; i<len; i++){
			    ...
			    var value = {};
				if(self.trHeight==0){
			    	self.trHeight = $tr.attr("lineHeight");
			    }
			    for(var j=0,jlen=tds.length; j<jlen; j++){
				      ...
				      if(self.hiddenColumn[colId]){  //如果是隐藏列，放入隐藏列值集合中
				    	  value[colId]= $.trim($.text($td));
				      }else{
				    	  cells.push({colId:colId,rowIndex:i,index:j,$td:$td,html:$.trim($.text($td)),value:colValue});
				      }
			    }
		   ...
		   * 创建当前无数据
		   **/
		  self.createNoDate = function(){
			  var height = self.$gridbodycage.height()-50;
			  self.$gridbody.append('<tr nodate="nodate"><td class="nodata" height="'+height+'" width="100%" align="center"><span class="nodata-l"> <span class="nodata-r"> <span class="nodata-m"> <span class="icon">当前无数据</span> </span></span></span></td></tr>');
		  }
}
...
function index_gridpanel_DomCtrlFn(conf){
	  ...
	  self.addRows = function(conf,index){
		 var $tr = $('<tr></tr>');
		 if(self.getTrHeight()){
			 var trheight = self.getTrHeight();
			 if(trheight.indexOf('px') < 0){
				 trheight+="px";
			 }
	    	 $tr.attr("height",trheight);
	     }
	  ...
	  self.getTrHeight = function(){
			 return  self.$gridbody.attr("lineHeight");
		  }
	  //添加空行
	  ...
	  self.addCells = function(conf,rowId,$row){
		  ...
		  for(var i=0,len = self.cols.length;i<len;i++){
			  ...
			  var $td = $("<td></td>").append($span).attr("colId",colId);
			  var tdObj = null;
      ...
      self.overLine = function(){
      	    var $tbody = self.$gridbody;
	    	$tbody.delegate("tr", "mouseover",function(e) {
	    	    if($(this).attr("nodate")===undefined){
	    			$(this).addClass("blue");
	    		}
	    	});
	        $tbody.delegate("tr", "mouseout",function(e) {
	    	    $(this).removeClass("blue");
	    	});
	    /*
	      var $tr = self.$gridbody.children("tr");
	      $tr.lazybind("mouseover",function(param){
	      	    document.onmousemove = null;
	    		if($(param.self).attr("nodate")===undefined){
	    			$(param.self).addClass("blue");
	    		}
	    	}, 200,'mouseout');
    ...
    self.delScrollColumn = function(){
    	if(self.scrollTh && self.$gridhead){
    		self.$gridhead.remove(self.scrollTh);
    		//self.$gridhead = null;
    	}
    };
    ...
    //斑马线
    self.banma = function(){
    	self.$gridbody.children("tr:even").removeClass("gray").addClass("white");
    	self.$gridbody.children("tr:odd").removeClass("white").addClass("gray");
    };
...
function index_gridpanel_ComponetFn(conf){
   ...
   */
  self.rend = function(rendConf,start,callback){
	  ...
	  for(var jlen = self.rows.length; j<jlen; j++){//循环所有行
		  ...
		  for(var k=0,klen=cells.length; k<klen; k++){//循环列
			  ...
			  if(fn){
				  ...
				  // 获取当前单元格的宽度
				  var cellWidth = cell.$td.width();
				  // 设定悬浮提示
				  if(null != cellValue || (cell.$td.attr("class").indexOf("nodata") > -1)){
					  cell.$td.attr('title', cellValue);
				  }
			  }
		  }
	  }
   ...
   *rowIndex :num
   * */
  self.Light = function(rowIndex){
  	  var $tr = $(self.$gridbody.children("tr")[rowIndex]);
	  $tr.addClass("blue");
	  $tr.unbind("mouseout");
  };
   ...
   *rowIndex :num
   * */
  self.cancelLight = function(rowIndex){
  	  var $tr = $(self.$gridbody.children("tr")[rowIndex]);
	  $tr.removeClass("blue");
	  $tr.bind("mouseout",function(){
		  $tr.removeClass("blue");
  ...
  self.loadGridData = function(data){
       ...
       if(len == 0){
    	   ...
    	   self.checkOverFlow();
    	   self.rend();
		   self.trHeight=$(self.$gridbody.children('tr')[0]).height();
  ...
  self.clearAllGridData = function(){
	  self.rows= [];
      var $row = null;
      self.$gridbody.find("*").unbind();
      self.$gridbody.empty();
...
function SortPluginIndex(conf){
  ...
  for(var i=0;i<conf.sortColumns.length;i++){
    ...
    var tmpSpan = $('<span class="left sort-ico"></span>');
    tmpSpan.append($upa).append($downa);
    headObj.$head.children("span").append(tmpSpan).bind({"mouseover":bindMouseOver,"mouseout":bindMouseOut});;
  }
  ...
  function bindMouseOut(){
    ...
    for(var i = 0,len = sortAs.length ; i < len;i++){
      var $sortA = $(sortAs[i]);
