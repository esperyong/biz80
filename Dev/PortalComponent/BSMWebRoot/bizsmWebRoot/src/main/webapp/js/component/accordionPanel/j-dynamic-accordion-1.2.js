...
function defatule_JDynamicAccordionPanel_ComponetFn(conf){
	...
	this.addInnerComponetSlab = function(slabInfo, slabICOClassNameArray, innerElement, userData, clickCallFunc){
		...
		var $slabSpan = $("<nobr><span nameField='nameField' class='"+settings.panelTitleClassName+"' title='"+slabInfo.label+"'>"+slabInfo.label+"</span></nobr>");
		if(settings.labelOverflowWidth != null 
			&& $.trim(settings.labelOverflowWidth) != ""){
			$slabSpan = $("<nobr><span nameField='nameField' class='"+settings.panelTitleClassName+"' style='display:inline-block;width:"+settings.labelOverflowWidth+";overflow:hidden;border:0px solid red;text-overflow:ellipsis;' title='"+slabInfo.label+"'>"+slabInfo.label+"</span></nobr>");
		}
	 ...
	 */
	this.addSlabTitleToolBtn = function(slabID, btnInfo, userData, clickCallFunc){
		...
		if(clickCallFunc != null && clickCallFunc != ""){
			$icoBtn.click(function(event) {
				...
				paramMap["slabID"] = btnInfo["slabID"];
				paramMap["btnID"] = btnInfo["btnID"];
				paramMap["icoClassName"] = btnInfo["icoClassName"];
