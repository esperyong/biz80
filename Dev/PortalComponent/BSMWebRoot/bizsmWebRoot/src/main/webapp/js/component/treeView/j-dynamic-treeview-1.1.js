	...
	*/
	this.setLiNode = function($liNode, nodeInfo, nodeSelected, userData, clickCallFunc){
		...
		if(clickCallFunc != null && clickCallFunc != ""){
			$liNode.find('>nobr>span').hover(function(){
				$(this).addClass('hover');
			}, function(){
				$(this).removeClass('hover');
			});
		}
		$liNode.find('>nobr>span').click(function(event){
			var $this = $(this);
			if(nodeSelected){
				var $oldSelected = treeObjectTemp.find("li>nobr>span[selected='true']");
	...
	this.moveSubNodeToNode = function(nodeID, $liTemp){
		...
		if($subNodes.size() > 0){
			...
			$liTemp.find('>nobr>span').hover(function(){
				$(this).addClass('hover');
			}, function(){
				$(this).removeClass('hover');
			});
			...
			//为节点span元素添加click事件
			$liTemp.find('>nobr>span').toggle(function(event){
				...
				if(treeSettingsTemp.unique == true && treeSettingsTemp.collapsed == true){
					...
					if($expandableLiOld.size() > 0){
						$expandableLiOld.find('>nobr>span').click();
					}
				...
				//折起该节点下面,所有处于展开的节点
				$thisLi.find('>ul li.collapsable>nobr>span,>ul li.lastCollapsable>nobr>span').click();
				if(treeSettingsTemp.animated != null && treeSettingsTemp.animated != ""){
			...
			$liTemp.find('>div').click(function(event){
				$(this).parent().find('>nobr>span').click();
				event.stopPropagation();
				return null;
			});
		}
...
function defatule_JDynamicTreeview_ComponetFn(conf){
	...
	this.appendToContainer = function(container){
		if(settings.collapsed == false){
			...
			settings.animated = "";
			globalObj.treeObject.find('li>nobr>span').click();
	...
	this.calcCount = function(){
		...
		$liNode.each(function(cnt){
			...
			if($this.attr("isDataNode") != "true"){
				...
				$this.find('>nobr>span').text($this.find('>nobr>span').text()+"("+totalCount+")");
			}
		});
	}
	/**
	...
	*/
	...
	this.calcCount = function(){
		...
		$liNode.each(function(cnt){
			...
			var $subLiNode = $this.find('li[isDataNode="true"]');
			var totalCount = 0;
			$subLiNode.each(function(subCnt1){
				var $thisSub = $(this);
				var subCountTemp = $thisSub.find('>nobr>span').attr("subCount");
				totalCount = totalCount*1+subCountTemp*1;
			});
			if($this.attr("isDataNode") != "true"){
				$this.attr("subCount", totalCount);
				$this.find('>nobr>span').text($this.find('>nobr>span').text()+"("+totalCount+")");
	...
	*/
	this.loadSubCount = function(){
		...
		$liNode.each(function(cnt){
			...
			if($this.attr("isDataNode") != "true"){
				...
				$this.find('>nobr>span').text($this.find('>nobr>span').text()+"("+subCount+")");
			}
		});
	}
	/**
	...
	/**
	 * 添加树组件节点内容
	 * param JSONObject nodeInfo（nodeID, label, parentNodeID, isDataNode, subCount, imgPath)
	 * param boolean nodeSelected 是否添加node选中效果
	 * param JSONObject userData (用户数据对象)
	 * param Object clickCallFunc 点击节点触发的事件监听器函数
	 * 
		...
		if($parent_li_node.size() > 0){
			...
			if($parent_li_ul.size() > 0){
					...
					if(nodeInfo["isDataNode"] == "true" 
						...
						var subCntTemp = nodeInfo["subCount"];
						if(subCntTemp == null 
								|| subCntTemp == ""
								|| subCntTemp == "undefined"){
							subCntTemp = 0;
						}
						$liTemp.append('<span subCount="'+subCntTemp+'">'+nodeInfo.label+'('+subCntTemp+')</span>');
					}else{
					...
					*/
					...
					if(nodeInfo["isDataNode"] == "true" 
						...
						if(settings.labelOverWidth != ""){
							$liTemp.append('<nobr><span style="display:inline-block;width:'+settings.labelOverWidth+';overflow:hidden;border:0px solid red;text-overflow:ellipsis;" title="'+nodeInfo.label+'">'+nodeInfo.label+'</span></nobr>');
						}else{
							$liTemp.append('<nobr><span>'+nodeInfo.label+'</span></nobr>');
						}
					}else{
						$liTemp.append('<nobr><span>'+nodeInfo.label+'</span></nobr>');
					}
				...
				*/
				...
				$liTemp.find('>nobr>span').click(function(event){
					var $this = $(this);
					if(nodeSelected){
						var $oldSelected = globalObj.treeObject.find("li>nobr>span[selected='true']");
						$oldSelected.attr("selected", "false");
						$oldSelected.removeClass("selectednode");
...
//=================================================================================================================
				...
				$parent_li_node.find('>nobr>span').hover(function(){
					$(this).addClass('hover');
				}, function(){
					$(this).removeClass('hover');
				});
				...
				$parent_li_node.find('>nobr>span').toggle(function(event){
					var $thisLi = $(this).parent().parent();
	...
	this.nodeClick = function(nodeID, nodeSelected, clickCallFunc){
		globalObj.treeObject.find("li[nodeID='"+nodeID+"']>nobr>span").bind("click", function(){
			var $this = $(this);
