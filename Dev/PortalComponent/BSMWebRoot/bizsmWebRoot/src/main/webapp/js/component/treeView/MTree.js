...
MTree.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  var HTML = "<DIV noWrap='True' style='height:20px;'><NOBR>";
  if(!isRoot)
  {
    ...
    else
    {
      ...
      HTML += "<SPAN style='float:left;'>"+ this.word2image(node.parentNode.childAppend) +"<IMG "+
  ...
  var cfunciton = this.getAttribute(source, "cfunciton"); // yeqw 用户自定义函数属性
	...
	if("checkbox" == ctext) {	
		  HTML +="<span "+
			...
			">"+ "<INPUT TYPE='checkbox' id='"+node.cid+"' NAME='"+node.cname+"' onclick='checkall(this)' value='"+node.cvalue+"' nodelength='"+node.path+"' treename='"+this.name+"' cfunciton='"+node.cfunciton+"'>"+pimg+node.text + 
		  "</span></NOBR></DIV>";
	} else if("radio" == ctext) {	
		  HTML +="<span "+
			"class='MzTreeview' hideFocus "+
  ...
  { 
    ...
    if(!node.isLoad)
    {
      ...
      if(node.hasChild && node.childNodes.length>this.nodecountToPrompt)
      {
        ...
        var temp = this.getElementById(this.name +"_tree_"+ id).childNodes;
        temp[temp.length-1].innerHTML = "<DIV noWrap style='height:20px;'><NOBR><SPAN>"+ (this.wordLine ?
