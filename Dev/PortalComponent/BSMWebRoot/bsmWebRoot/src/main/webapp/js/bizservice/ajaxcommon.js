...
function get_form_params(p_formId){
			...
			function (index, domEle) { 
				  ...
				  for(var i = 0,il = options.length;i < il;i++){
					if(options[i].selected){
						params += ((params.length > 0)?'&':'') + selectid + '='+options[i].value;
					}
				  }
			}
	);
	...
	$(inputSelector).each(
			function (index, domEle) { 
				  ...
				  if(type=='text'||type=='password'||type=='hidden'||(type=='checkbox'&&domEle.checked)){
					  params += ((params.length > 0)?'&':'')+domEle.id+'='+domEle.value;
				  }
				  if(type=='radio'&&domEle.checked){
					  params += ((params.length > 0)?'&':'')+domEle.name+'='+domEle.value;
				  }
			}
	);
	...
	$(textareaSelector).each(
			function (index, domEle) { 
				  params += ((params.length > 0)?'&':'')+domEle.id+'='+$(domEle).val();
			}
	);		
