...
var unityObject = function() {
				...
				if (ua.win) {
					if (ua.ff || ua.ch) {
						...
						if (typeof ActiveXObject != "undefined") {
							...
							return axTest("1.7.0") || axTest("1.6.0") || axTest("1.5.0") || axTest("1.4.2");
						}
					}
				}
			}
	...
	function trackStatus(status) {
		...
		if (!analyticsLoaded) {
			...
			_gaq.push(["unity._setCustomVar", 1, "Revision", 51248, 2]);
		}
	...
	function detectUnity(callback) {
		...
		if (typeof nav.plugins != "undefined" && nav.plugins[pluginName] && typeof nav.mimeTypes != "undefined" && nav.mimeTypes[pluginMimeType] && nav.mimeTypes[pluginMimeType].enabledPlugin) {
			...
			if (/Safari/.test(nav.appVersion) && /Mac OS X 10_6/.test(nav.appVersion)) {
	...
	function createMissingUnity(id, attributes, params, callback, broken) {
		...
		if (re) {
			var type = "standard";
			if (enableJava && ua.java && !triedJava) {
				applets[id] = {
				...
				var url = hostURL+"/flash/UnityWebPlayer.exe";
				
			}
			else if (nav.platform == "MacIntel") {
				var url = baseUrl + "webplayer-i386.dmg";
			}
			else if (nav.platform == "MacPPC") {
				var url = baseUrl + "webplayer-ppc.dmg";
			}
			else {
			...
			if (broken) {
				...
				var img = hostURL+"/flash/pic/getunity.png";
				var imgWidth = 190;
				var imgHeight = 75;
			}
			else {
			...
			if (ua.win && ua.ie) {
				...
				if (fullSizeMissing) {
					...
					re.outerHTML = '<div id="' + id + '" style="width: ' + appendPX(width) + '; height: ' + appendPX(height) + '; text-align: center;">' + ft + '</div>';
				}
				else {
					re.outerHTML = '<div id="' + id + '">' + at + '</div>';
				}
			}
			else {
				var ue = doc.createElement("div");
	...
	function doJavaInstall(id) {
		...
		var p = {
			...
			installer : baseUrl + getWinInstall(),
			image : "../flash/pic/getunity.png",//"http://webplayer.unity3d.com/installation/unitylogo.png",
			centerimage : true,
			boxborder : false,
			scriptable : true,
			mayscript : true
