...
function noAccvalidateTimeRange(tag) {
	...
	if(self){
		if(self.startValue<self.endValue){
		  ...
		  if(selfid.indexOf("startTime")!=-1 ){
		    if($.validate($parentTd.find('input[id$=endTime]'))){
		      var endTimeId = $parentTd.find('input[id$=endTime]').attr("id");
		      endTimeId = endTimeId.replace(/\[/g,""); 
		      endTimeId = endTimeId.replace(/\]/g,"");
		      $("."+endTimeId+"formError").click();
		    }
		  }else if(selfid.indexOf("endTime")!=-1 ){
		    if($.validate($parentTd.find('input[id$=startTime]'))){
		       var startTimeId = $parentTd.find('input[id$=startTime]').attr("id");
		       startTimeId = startTimeId.replace(/\[/g,""); 
		       startTimeId = startTimeId.replace(/\]/g,"");
		       $("."+startTimeId+"formError").click();
		    }
		  }
			return false;
		}
...
function getNatExactTimePeriod(index){
	...
	var startTime = '<input name="nonAccessTimes[' + index + '].startTime" id="nonAccessTimes[' + index + ']_startTime" size="6" value="'+_startTime+'" class="validate[funcCall[noAccStartTimeLarge]]"/>';
	var endTime = '<input name="nonAccessTimes[' + index + '].endTime"  id="nonAccessTimes[' + index + ']_endTime" size="6" value="'+_endTime+'" class="validate[funcCall[noAccEndTimeSmall]]"/>';
	var exact = '<input name="nonAccessTimes[' + index + '].date" size="10" id="exactTime[' + index + ']" class="validate[required]"/>';
	var temp = $('<div></div>');
...
function delLine(){
  var $selectRows = $('#natPanel').children(':gt(0)').find(':checked');
  // 清除删除行的验证提示
  $selectRows.each(function(){
    var $startTime = $(this).parentsUntil('li').parent().find('input[id$=startTime]');
    var $endTime = $(this).parentsUntil('li').parent().find('input[id$=endTime]');
    var startTimeId = $startTime.attr("id");
    startTimeId = startTimeId.replace(/\[/g,""); 
    startTimeId = startTimeId.replace(/\]/g,"");
    $("."+startTimeId+"formError").click();
    var endTimeId = $endTime.attr("id");
    endTimeId = endTimeId.replace(/\[/g,""); 
    endTimeId = endTimeId.replace(/\]/g,"");
    $("."+endTimeId+"formError").click();
  });
  $selectRows.parentsUntil('li').parent().remove();
