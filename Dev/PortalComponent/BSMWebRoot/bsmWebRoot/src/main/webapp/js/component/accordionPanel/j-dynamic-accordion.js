...
function defatule_JDynamicAccordionPanel_ComponetFn(conf){
	 ...
	 */
	this.addInnerComponetSlab = function(slabInfo, innerElement, userData, clickCallFunc){
		var $jAccordionSlab = $("<h3 slabID='"+slabInfo.slabID+"'><a href='#'><div class='"+settings.panelTopClassName+"'><div class='"+settings.panelIcoClassName+"'/><span nameField='nameField' class='"+settings.panelTitleClassName+"' style='color:#fff'>"+slabInfo.label+"</span></div></a></h3>");
	 ...
	 * param function clickCallFunc (点击Slab折叠页签回调函数)
	 */
	this.addPageSlab = function(slabInfo, path, userData, clickCallFunc){
		var $jAccordionSlab = $("<h3 slabID='"+slabInfo.slabID+"'><a href='#'><div class='"+settings.panelTopClassName+"'><div class='"+settings.panelIcoClassName+"'/><span nameField='nameField' class='"+settings.panelTitleClassName+"'>"+slabInfo.label+"</span></div></a></h3>");
		var $userDataDiv = $('<div jAccordionUserDataArea="true"/>');
		$userDataDiv.hide();
		if(userData != null && userData != ""){
			for(var key in userData){
		...
		$jAccordionSlab.click(function(event) {
			...
			if($iframeObj.size() == 0){
				$iframeObj = $("<iframe elID='jAccordionContentIframe' frameborder='NO' border='0' scrolling='yes' noresize framespacing='0' style='width:100%; height:100%'/>");
