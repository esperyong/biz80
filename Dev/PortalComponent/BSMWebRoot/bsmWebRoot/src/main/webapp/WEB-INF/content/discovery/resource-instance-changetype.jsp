...
$(document).ready(function() {
	...
	$("#sp_discovery").bind("click", function() {
		...
		$("#change_type_result_div").hide();
		$.ajax( {
			type : "post",
			url : "resource-instance-changetypesubmit.action",
			data : "groupId="+groupId+"&instanceId=${instanceId}",
			success : function(data, textStatus) {
				var errorMessage = data.changeResult[0].errorMessage;
				if(errorMessage != null){
					$("#change_type_result_ico").attr("class","ico ico-false");
					$("#change_type_result_ico").attr("title","失败");
				}else{
					$("#change_type_result_ico").attr("class","ico ico-right");
					$("#change_type_result_ico").attr("title","成功");
				}
				
				stopLoading();
				$("#change_type_result_div").show();
				
				if($("#changeModuleONOFF").attr("checked")){
	...
	if($("#changeModuleONOFF").length != 0){
		...
		$("#changeModuleONOFF").bind("click", function(){
			if(this.checked){
				...
				$("#discovery-info-div").show();
				$("#dis-info-div").show();
			}else{
				var url = "resource-instance-changetype.action?instanceId=${instanceId}";
				var formObj = document.getElementById("form1");
				formObj.target="";
				formObj.action = url;
				formObj.submit();
    ...
    <page:applyDecorator name="accordionPanel">  
		...
		<page:param name="display"></page:param>
	  	<page:param name="content">
			...
			<ul class="textalign">
				<li style="width:80%;" class="margin5 padding2"><span style="float: left; padding-top:5px;">&nbsp;选择设备类型：</span><span>${parentGroupSelect }<s:select list="groupList" id="groupId" name="groupId" isSynchro="1" listKey="resourceCategoryGroupId" listValue="resourceGroupName" value="%{groupId}" offsetWidth="150"></s:select></span></li>
				<!-- 没有模型的是一种显示方式，有模型的按是否监控来区分显示内容 -->
				<s:if test="resourceId == null">
					<li style="width:80%;" class="margin5 padding2"><span class="red">&nbsp;当前资源无对应模型，可选择监控模型进行重新发现。</span></li>
				</s:if>
				<s:else>
					<li style="width:80%;" class="margin5 padding2"><span>&nbsp;当前监控模型：<s:property value="resourceName"/></span></li>
				</s:else>
				<li style="width:80%;" class="margin5 padding2"><input type="checkbox" id="changeModuleONOFF">同时变更监控模型</li>
				...
				<li style="width:80%;">
				  	<ul>
						<li class="margin3 padding2">
								<div id="dis-info-div" style="display:none;" class="greywhite-border padding5">
									<div class="h2"></span>发现信息</div>
									<div id="discovery-info-div" style="position:relative;display:none;">
		  	...
		  	<page:param name="content">
		  		<div id="disc_result">
					<page:applyDecorator name="resource-discovering">
						<page:param name="insertContent">
							<div class="h3" id="change_type_result_div" style="display:none;">
								<span class="bold">变更设备类型结果：</span>
								<span id="change_type_result_ico"></span>
							</div>
						</page:param>
