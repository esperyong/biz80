...
<script language="javascript">
	$(function() {
		var vToolbar = new JDynamicVerticalToolbar({id: "jVerticalToolbar-1", height: "623px"});
		vToolbar.addItem({itemID: "item1", title: "业务服务管理", className: "service-tool-ico-navi"}, {test:"aaa"}, function(dataMap){
			...
			parent.parent.document.all.rightFrame.src = "";
			//alert(dataMap["test"]);
		});
		vToolbar.addItem({itemID: "item2", title: "业务服务监控", className: "service-tool-ico-jk"}, {test:"bbb"}, function(dataMap){
			parent.parent.document.all.rightFrame.src = "${ctx}/bizsm/bizservice/ui/bizservice-monitor";
			//alert(dataMap["test"]);
		});
		vToolbar.addItem({itemID: "item3", title: "业务服务影响", className: "service-tool-ico-yx"}, null, function(dataMap){alert(dataMap.title);});
		vToolbar.addItem({itemID: "item4", title: "业务服务分析", className: "service-tool-ico-fx"}, null, function(dataMap){alert(dataMap.title);});
		vToolbar.addItem({itemID: "item5", title: "业务服务一览", className: "service-tool-ico-yl"}, null, function(dataMap){alert(dataMap.title);});
