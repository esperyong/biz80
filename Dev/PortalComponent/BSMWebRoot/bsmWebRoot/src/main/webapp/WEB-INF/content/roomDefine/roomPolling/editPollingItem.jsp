<!-- 机房-机房巡检-编辑巡检模板-编辑检查项 editPollingItem.jsp -->
...
<page:param name="title">编辑检查项 </page:param>	
...
<form id="editPollingItemFormID" action="" name="editPollingContentForm" method="post" >
...
<div class="pop-m">
	...
	<span class="black-btn-l right"><span class="btn-r"><span class="btn-m" id="delBtn"><a>删除</a></span></span></span>
      ...
      <td><input type="hidden" name="contentId" id="contentId" value=""/></td>
...
$("#submit").click(function (){
	...
	if(trs.length==2){
		window.toast.addMessage("没有检查项");
		return;
	}else{
		$("#editPollingItemFormID").attr("action","${ctx}/roomDefine/EditPollingItemVisit!editPollingItem.action");
		$("#editPollingItemFormID").attr("target","submitIframe");
		//$("#tb tr:gt(0)").each(function() {
	    //    $(this).find("#CK").get(0).checked = true;
	    //});
		$("#editPollingItemFormID").submit();
	}
});
...
$(document).ready(function() {
	$("#editPollingItemFormID").validationEngine({
...
function setval(indexNo,pollingContentId,desc,pollingContent) {
	...
	var pollingContentIdObj = $("#pollingContentId input[name='contentId']");
